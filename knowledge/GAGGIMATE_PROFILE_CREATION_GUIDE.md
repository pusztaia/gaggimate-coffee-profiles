...
### Firmware behaviors observed on-device (Jul 2026, originally on fw 1.8.0; structurally re-confirmed against the `gaggimate-source` firmware tree for 1.8.1)

- **`phase: "decline"` is normalized to `"brew"`** — confirmed in `src/display/models/profile.h`: the `PhaseType` enum only has `PHASE_TYPE_PREINFUSION`/`PHASE_TYPE_BREW`; any `"phase"` value other than `"preinfusion"` (including `"decline"`) parses to `BREW`, and serialization only ever writes back `"preinfusion"` or `"brew"`. This is a schema-level fact of the parser, not tied to a specific patch version — author repo JSONs with `"brew"` for decline phases so repo and device stay byte-identical.
- **`transition.duration` above the documented 10s max executes correctly** — confirmed in source: `transition.duration` is read as an unclamped `float`, with no upper-bound check anywhere in the parser. 38–40s linear declines running as authored (telemetry-verified on 1.8.0) is consistent with this. The 0–10s range in the transition table is stale documentation for ramp-style transitions, not a firmware limit.
- **A pump-off phase encodes as `{"target": "flow", "pressure": 0, "flow": 0}`** — the `pro`-profile `PumpTarget` enum only has `PUMP_TARGET_FLOW`/`PUMP_TARGET_PRESSURE` (no `"power"`/`"off"` at this level); `"power"` is only meaningful for `type: "standard"` profiles' simple integer `pump` field. flow/0/0 is the pattern the current firmware profiles use for a bloom/soak phase.
- The **`pumped` backstop margin numbers** (~90–115 ml normal total, 130 ml backstop for a 22g/55g shot) are empirical measurements from the Jul 2026 fw-1.8.0 test, not re-measured on 1.8.1 — treat the *mechanism* (pair volumetric with a `pumped` backstop) as solid, since it's built on the schema-level facts above, but re-verify the *numbers* on your own machine before trusting them as a safety margin.
