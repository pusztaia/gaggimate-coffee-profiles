...
The **Automatic Pro** is a firmware built-in profile maintained by the Gaggimate developer — not an agent-created profile. It ships pre-installed on Gaggimate Pro devices in multiple dose variants (16g, 18g, 20g, 22g). *(Empirical/unverified in source: the `gaggimate-source` firmware tree checked for this repo only shows `ProfileManager.cpp` auto-creating a single generic `"Default"` standard-type profile at first boot — no bundled "Automatic Pro" JSON or `vIT3` label was found in the source dump, so this claim rests on device/community observation, not the firmware source.)*
