...
*(Partially confirmed in source, mechanism only: `src/display/core/Controller.cpp` (`onVolumetricMeasurement`, `VolumetricMeasurementSource::FLOW_ESTIMATION`) confirms the firmware does fall back to a non-Bluetooth "flow estimation" volumetric source — triggered automatically when `isBluetoothScaleHealthy()` is false — and feeds it through the same `updateVolume()` path as a real scale reading. However, the actual estimation math (whether it's pressure-curve analysis, flow integration, time modeling, or some combination) is computed on a separate sub-controller board and isn't present in this `src/display/` source tree, so the specific three-technique breakdown above is unverified against source — treat it as an external/empirical description of behavior, not a source-confirmed algorithm.)*
...
- Based on Gaggimate firmware v1.6.0+ *(unverifiable in source: the `gaggimate-source` tree has no checked-in `version.h` — the firmware version string is generated at build time from git tags, so this specific version number can't be cross-checked against the source dump; treat it as the author's empirical recollection)*
