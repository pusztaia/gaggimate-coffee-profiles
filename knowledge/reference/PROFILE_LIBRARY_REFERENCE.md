...
```json
{
  ...
  "phases": [
    ...
    },
    {
      ...
      "temperature": 0,
      "transition": { "type": "instant", "duration": 0, "adaptive": true },
      "pump": { "target": "flow", "pressure": 0, "flow": 0 }
    },
    {
      "name": "Ramp",
      "phase": "brew",
...
> **Firmware note:** The Bloom phase's pump target is `"flow"`, not `"power"` — confirmed against `src/display/models/profile.h`: the `pro`-profile `PumpTarget` enum only has `PUMP_TARGET_FLOW`/`PUMP_TARGET_PRESSURE`. `"power"` only exists for `type: "standard"` profiles' plain-integer `pump` field; in a `pro` phase, an unrecognized `target` string silently falls back to `"flow"` (`profile.h:303`), so `flow: 0` still yields a pump-off bloom, but the JSON should say `"flow"` to match what the firmware round-trips. See `../GAGGIMATE_PROFILE_CREATION_GUIDE.md`'s "Firmware behaviors observed on-device" section.
...
> **Firmware note:** Written here as `"phase": "brew"` rather than `"decline"` — the `PhaseType` enum in `src/display/models/profile.h` only has `PHASE_TYPE_PREINFUSION`/`PHASE_TYPE_BREW`; any other string (including `"decline"`) is normalized to `"brew"` at parse/save time, so authoring `"decline"` and `"brew"` are functionally identical but only `"brew"` matches what the firmware writes back on export. See `../GAGGIMATE_PROFILE_CREATION_GUIDE.md`.
...
> **Firmware note:** Same fix as above — `"phase": "brew"` (not `"decline"`) and `"target": "flow"` on the Bloom phase, per the `PhaseType`/`PumpTarget` enums in `src/display/models/profile.h`.
